...
import com.wavefront.sdk.jaxrs.app.SampleApp;
...
/**
 * Test class to test reported metric/histogram for Dropwizard (JAX-RS) apps requests/responses
 *
 * @author Sushant Dewan (sushant@wavefront.com).
 */
public class JaxrsAppReporterTest {
  ...
  private void testCreate() throws IOException {
    ...
    Map<String, String> tags = new HashMap<String, String>() {{
      ...
      put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
      put("jaxrs.resource.method", "barCreate");
    }};
    // Request gauge
    ...
    expectedTags.add(new Pair<>("component", "jaxrs-server"));
    ...
    expectedTags.add(new Pair<>("jaxrs.resource.class",
        "com.wavefront.sdk.jaxrs.app.SampleApp.SampleResource"));
    expectedTags.add(new Pair<>("jaxrs.path", "sample/foo/bar"));
  ...
  private void testRead() throws IOException {
    ...
    Map<String, String> tags = new HashMap<String, String>() {{
      ...
      put(SHARD_TAG_KEY, SampleApp.SHARD);
      put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
      put("jaxrs.resource.method", "barGet");
    }};
    // Request gauge
    assertEquals(0, sampleApp.reportedValue(new MetricName(
    ...
    expectedTags.add(new Pair<>("jaxrs.resource.class",
        "com.wavefront.sdk.jaxrs.app.SampleApp.SampleResource"));
    expectedTags.add(new Pair<>("jaxrs.path", "sample/foo/bar/{id}"));
    expectedTags.add(new Pair<>("application", "wavefront"));
    expectedTags.add(new Pair<>("service", SampleApp.SERVICE));
    expectedTags.add(new Pair<>("cluster", SampleApp.CLUSTER));
    expectedTags.add(new Pair<>("shard", SampleApp.SHARD));
  ...
  private void testUpdate() throws IOException {
    ...
    Map<String, String> tags = new HashMap<String, String>() {{
      ...
      put(SHARD_TAG_KEY, SampleApp.SHARD);
      put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
      put("jaxrs.resource.method", "barUpdate");
    }};
    // Request gauge
    assertEquals(0, sampleApp.reportedValue(new MetricName(
  ...
  private void testDelete() throws IOException {
    ...
    Map<String, String> tags = new HashMap<String, String>() {{
      ...
      put(SHARD_TAG_KEY, SampleApp.SHARD);
      put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
      put("jaxrs.resource.method", "barDelete");
    }};
    // Request gauge
    assertEquals(0, sampleApp.reportedValue(new MetricName(
...
//    assertEquals(200, invokeGetRequest("sample/foo/bar/1/1"));
    ...
    Map<String, String> tags = new HashMap<String, String>() {{
      ...
      put(SHARD_TAG_KEY, SampleApp.SHARD);
      put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
      put("jaxrs.resource.method", "getAll");
    }};
    // Request gauge
    assertEquals(0, sampleApp.reportedValue(new MetricName(
...
//    assertEquals(1, sampleApp.reportedValue(new MetricName(
//        "response.sample.foo.bar._id_._subid_.GET.200.cumulative", tags)));
  ...
  private void testGetDuplicatePathArgs() throws IOException {
    assertEquals(200, invokeGetRequest("sample/foo/bar/1/1/1"));
//    @Path("/bar/{id}/{subId}/{subId1}/{subId}")
    Map<String, String> tags = new HashMap<String, String>() {{
      put(CLUSTER_TAG_KEY, SampleApp.CLUSTER);
      put(SERVICE_TAG_KEY, SampleApp.SERVICE);
      put(SHARD_TAG_KEY, SampleApp.SHARD);
      put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
      put("jaxrs.resource.method", "getMultiplePathArgs");
    }};
    // Request gauge
    assertEquals(0, sampleApp.reportedValue(new MetricName(
        "request.sample.foo.bar._id_._subId_._id_.GET.inflight", tags)));

    // Response counter metric
    assertEquals(1, sampleApp.reportedValue(new MetricName(
        "response.sample.foo.bar._id_._subId_._id_.GET.200.cumulative", tags)));

    // Aggregated metrics (delta counters)
    assertEquals(1, sampleApp.reportedValue(new MetricName(
        "response.sample.foo.bar._id_._subId_._id_.GET.200.aggregated_per_application",
        new HashMap<String, String>() {{
          put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
          put("jaxrs.resource.method", "getMultiplePathArgs");
          put("source", WAVEFRONT_PROVIDED_SOURCE);
    ...
    assertEquals(1, sampleApp.reportedValue(new MetricName(
        "response.sample.foo.bar._id_._subId_._id_.GET.200.aggregated_per_cluster",
        new HashMap<String, String>() {{
          put(CLUSTER_TAG_KEY, SampleApp.CLUSTER);
          put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
          put("jaxrs.resource.method", "getMultiplePathArgs");
    ...
    assertEquals(1, sampleApp.reportedValue(new MetricName(
        "response.sample.foo.bar._id_._subId_._id_.GET.200.aggregated_per_service",
        new HashMap<String, String>() {{
          put(CLUSTER_TAG_KEY, SampleApp.CLUSTER);
          put(SERVICE_TAG_KEY, SampleApp.SERVICE);
          put("jaxrs.resource.class", SampleApp.SampleResource.class.getCanonicalName());
    ...
    assertEquals(1, sampleApp.reportedValue(new MetricName(
        "response.sample.foo.bar._id_._subId_._id_.GET.200.aggregated_per_shard",
        new HashMap<String, String>() {{
          put(CLUSTER_TAG_KEY, SampleApp.CLUSTER);
          put(SERVICE_TAG_KEY, SampleApp.SERVICE);
          put(SHARD_TAG_KEY, SampleApp.SHARD);
    ...
    // Response latency histogram
    assertEquals(1, sampleApp.reportedValue(new MetricName(
        "response.sample.foo.bar._id_._subId_._id_.GET.200.latency", tags)));
    assertEquals(1, sampleApp.reportedValue(new MetricName(
        "response.sample.foo.bar._id_._subId_._id_.GET.200.cpu_ns", tags)));

    // Tracing Span
    WavefrontSpan span = sampleApp.reportedSpan("getMultiplePathArgs");
    assertNotNull(span);
    List<Pair<String, String>> expectedTags = new ArrayList<>();
    expectedTags.add(new Pair<>("span.kind", "server"));
    expectedTags.add(new Pair<>("component", "jaxrs-server"));
    expectedTags.add(new Pair<>("http.method", "GET"));
    expectedTags.add(new Pair<>("http.url",
        "http://localhost:" + sampleApp.getHttpPort() + "/sample/foo/bar/1/1/1"));
    expectedTags.add(new Pair<>("http.status_code", "200"));
    expectedTags.add(new Pair<>("jaxrs.resource.class",
        "com.wavefront.sdk.jaxrs.app.SampleApp.SampleResource"));
    expectedTags.add(new Pair<>("jaxrs.path", "sample/foo/bar/{id}/{subId}/{id}"));
    expectedTags.add(new Pair<>("application", "wavefront"));
    expectedTags.add(new Pair<>("service", SampleApp.SERVICE));
    expectedTags.add(new Pair<>("cluster", SampleApp.CLUSTER));
    expectedTags.add(new Pair<>("shard", SampleApp.SHARD));
  ...
  private void testOverallAggregatedMetrics() {
    // jaxrs.server.total_requests.inflight gauge should be 0
    assertEquals(0, sampleApp.reportedValue(new MetricName(
